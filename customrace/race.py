...
def is_valid_emoji(emoji_str, guild):
    ...
    if match:
        ...
        if match.group(1) == "a":
            ...
            emoji = discord.utils.get(guild.emojis, id=emoji_id)
        return emoji is not None and emoji.guild == guild
    else:
        return False
...
class Race(commands.Cog):
    ...
    async def racers(self, ctx):
        """Show a list of all racers available."""
        racer_types = ["fast", "steady", "abberant", "predator", "special", "slow"]
        racers_by_type = {racer_type: [] for racer_type in racer_types}

        for racer in racers:
            emoji, racer_type = racer
            racers_by_type[racer_type].append(emoji)

        racer_list = []
        for racer_type in racer_types:
            if len(racers_by_type[racer_type]) > 0:
                racer_list.append(f"**{racer_type.capitalize()} Racers:**\n" + "\n".join(sorted(racers_by_type[racer_type])))

        if len(racer_list) == 0:
            await ctx.send("No racers available yet.")
        else:
            await ctx.send("\n\n".join(racer_list))
    ...
    @race.command()
    async def addracer(self, ctx, emoji: str, racer_type: str):
        """Add a new racer to the race."""
        # Check if the emoji is valid and if it is not already in the racers list
        guild = ctx.guild
        if not is_valid_emoji(emoji, guild):
            await ctx.send(f"The bot doesn't share a server with this emoji.")
            return
        if not is_valid_emoji(emoji, guild):
            await ctx.send("Invalid emoji.")
            return
        if any(emoji == racer[0] for racer in racers):
            await ctx.send("That emoji is already in use.")
            return

        # Add the new racer to the list
        racers.append((emoji, racer_type))
        await ctx.send(f"{emoji} added as a {racer_type} racer!")

    @race.command()
    async def delracer(self, ctx, emoji: str):
        """Remove a racer from the race."""
        global racers
        racers = [racer for racer in racers if racer[0] != emoji]
        await ctx.send(f"Racer with emoji {emoji} has been removed from the race.")
